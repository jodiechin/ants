...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        bees = nearest_place.bees
        x = 0 
        while len(bees) == 0 and nearest_place.entrance != hive:
    ...
    def action(self, colony):
        ...
        if QueenAnt.number_of_queens <= 1:
            ...
            while checker is not None:
                if checker.ant is not None:
                    if QueenAnt.fake.count(checker.ant) == 0:
                        QueenAnt.fake.append(checker.ant)
                        checker.ant.damage = 2 * checker.ant.damage
                    if checker.ant.container and checker.ant.ant is not None: 
                        if QueenAnt.fake.count(checker.ant.ant) == 0:
                            QueenAnt.fake.append(checker.ant.ant)
                            checker.ant.ant.damage = 2 * checker.ant.ant.damage
